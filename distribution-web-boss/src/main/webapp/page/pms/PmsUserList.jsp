...
<div class="pageContent">
	...
	<table class="table" targetType="navTab" asc="asc" desc="desc" width="100%" layoutH="130">
		...
		<tbody>
		    <c:forEach items="${pageBean.recordList}" var="record" varStatus="st">
		    	<%-- 普通用户看不到超级管理员信息 --%>
				...
				<tr target="sid_user" rel="${id}">
					...
					<td>
						<c:forEach items="${UserStatusEnumList}" var="userStatus">
							<c:if test="${record.status ne null and record.status eq userStatus.value}">${userStatus.desc}</c:if>
						</c:forEach>
					</td>
					<td>
						<c:forEach items="${UserTypeEnumList}" var="userTypeEnum">
							<c:if test="${record.userType ne null and record.userType eq userTypeEnum.value}">${userTypeEnum.desc}</c:if>
						</c:forEach>
					</td>
					<td>
						[<a href="pms_viewPmsUserUI.action?id=${record.id}" title="查看【${record.userNo }】详情" target="dialog" style="color:blue">查看</a>]
						<c:if test="${type eq UserTypeEnum.USER.value }">
						
						&nbsp;[<a href="pms_editPmsUserUI.action?id=${record.id}" title="修改【${record.userNo }】" target="dialog" rel="userUpdate" style="color:blue">修改</a>]
						<%-- 
						...
						--%>
						<c:if test="${type eq UserTypeEnum.USER.value }">
						&nbsp;[<a href="pms_deleteUserStatus.action?id=${record.id}" target="ajaxTodo" title="确定要删除吗？" style="color:blue">删除</a>]
						</c:if>
						</c:if>
					</td>
				</tr>
